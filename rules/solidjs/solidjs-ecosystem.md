---
paths: **/*.{ts,tsx,js,jsx}
description: "SolidJS 1.x library stack; the TanStack Solid adapters, stable-line version pinning against Solid 2.0 betas, solid-ui vendored components on Kobalte + corvu, Paraglide JS i18n, and cross-cutting adapter conventions."
---
...
For every concern below, use the TanStack Solid adapter before any alternative, including `@solidjs/router` and SolidStart (`@solidjs/start`). Alpha and beta packages are allowed; pin versions and read release notes when updating them.
...
- Messages are plain typed functions, callable from components and domain modules alike; no hook is required in the UI layer.
...
`@tanstack/solid-store` (alpha) is the state engine used by other TanStack libraries, not the application state layer — business state uses `solid-js/store` per the state architecture rules. When a TanStack library returns one of its Store instances, subscribe with `useSelector(store, selector)`; its `useStore` is deprecated in the Solid adapter.
