...
Business state and the logic that mutates it live in dedicated state modules, not in components. Components are thin views: they read exported accessors, call exported actions, and hold only UI-local state. Signals, stores, and memos work at module scope under `createRoot`, so no third-party state library is needed for this separation.
...
```ts
// Domain module: src/state/cart.ts
...
```ts
// Bad: any component can write arbitrary business state.
export const [cartState, setCartState] = createStore<CartState>({ items: [] });

// Good: writes happen only through domain verbs.
export const cart = createRoot(createCart);
```

## Components Stay Dumb

A component that fetches, derives, and mutates business state combines the view and viewmodel; it can only be tested by rendering it, and the logic cannot be reused. Components should read accessors, call actions, and render.

```tsx
// Bad: the component combines view and business logic.
...
// Good: the component renders the cart module's state.
const Cart: Component = () => (
  <section>
...
`createSignal` inside a component is for state that exists only for that view: open/collapsed flags, hover and focus, in-progress input drafts, and transient animation state. Use a state module when the value must survive unmount or navigation, another component needs it, or a business rule reads or constrains it.
